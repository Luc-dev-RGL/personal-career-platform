...
- **Design system « Bleu Nuit Éditorial »** : tokens CSS custom properties (`app/globals.css`) + Tailwind 4 (`@theme inline`) — fond bleu nuit (#141E34), texte ivoire (#F0EDE4), typographie Space Grotesk (display) / Inter (texte) / JetBrains Mono (labels). Composants UI maison (`components/ui/`) — pas de librairie de composants, contrôle total du style.
- **Palette sémantique** : chaque couleur porte un sens métier — orange #FF6B2C = marque & CTA, bleu info #6FA8FF = éléments techniques (tags, barres analytics), vert = succès & disponibilité, ambre = attente (rendez-vous pending, étape discussion), rouge = danger/perte. Tous les contrastes sont vérifiés AA/AAA sur fond bleu nuit (texte 14,2:1, accent 5,9:1, info 6,9:1). Le passage du noir au bleu nuit puis l'ajout de la sémantique n'ont coûté que des tokens + quelques classes : preuve que la centralisation rend le design système maintenable.
